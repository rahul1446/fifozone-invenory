...
const FlipkartAppPage = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await getOrdersApi({ platform: 'flipkart', limit: 20 });
    ...
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center text-2xl font-bold text-blue-600">F</div>
          <div><h1 className="text-2xl font-bold text-slate-800">Flipkart Integration</h1><p className="text-slate-500 text-sm mt-1">Orders and performance from Flipkart marketplace</p></div>
        </div>
        <Button icon={<RefreshCw size={16} />} onClick={fetchOrders}>Refresh</Button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total Orders', value: orders.length, icon: <ShoppingBag size={20} className="text-blue-600" />, border: 'border-blue-500', bg: 'bg-blue-50' },
          { label: 'Total Revenue', value: `\u20b9${totalRevenue.toLocaleString('en-IN')}`, icon: <IndianRupee size={20} className="text-indigo-600" />, border: 'border-indigo-500', bg: 'bg-indigo-50' },
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 font-semibold text-slate-700">Recent Flipkart Orders</div>
        <Table columns={columns} dataSource={orders} rowKey="_id" pagination={{ pageSize: 10 }} locale={{ emptyText: 'No Flipkart orders found' }} />
      </div>
    </div>
  );
};
