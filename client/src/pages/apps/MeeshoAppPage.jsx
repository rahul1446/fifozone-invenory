...
const MeeshoAppPage = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await getOrdersApi({ platform: 'meesho', limit: 50 });
  ...
  const fetchStatus = async () => {
    try {
      ...
      setConnected(s?.meesho?.connected || false);
      if (s?.meesho?.supplierId) form.setFieldsValue({ supplierId: s.meesho.supplierId });
  ...
  const handleSave = async () => {
    try {
      ...
      setSaving(true);
      await updateCredentialsApi('meesho', vals);
      message.success('✅ Meesho API credentials saved successfully!');
      setConnected(true);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Orders', value: orders.length, icon: <ShoppingBag size={20} className="text-pink-600" />, border: 'border-pink-400', bg: 'bg-pink-50' },
          { label: 'Total Revenue', value: `₹${totalRevenue.toLocaleString('en-IN')}`, icon: <IndianRupee size={20} className="text-rose-600" />, border: 'border-rose-400', bg: 'bg-rose-50' },
        ...
        <Tabs defaultActiveKey="orders" className="px-6 pt-2">
          <TabPane tab="📦 Orders" key="orders">
            {loading ? <div className="py-16 flex justify-center"><Spin size="large" /></div> :
              <Table columns={columns} dataSource={orders} rowKey="_id" pagination={{ pageSize: 15, showTotal: t => `${t} orders` }} locale={{ emptyText: '📭 No Meesho orders yet. Connect your API to start syncing.' }} />
            }
          </TabPane>
          ...
          <TabPane tab="🔑 API Settings" key="api">
            <div className="max-w-2xl py-4">
              <Alert
                message="Meesho Supplier API Credentials"
                description="Enter your Meesho Supplier API credentials to auto-sync orders and inventory. Get these from your Meesho Supplier Panel → API Access."
                type="info"
                showIcon
                className="mb-6"
              />
              <Form form={form} layout="vertical" requiredMark={false}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
                  <Form.Item label={<span className="font-semibold text-slate-700">Supplier ID</span>} name="supplierId" rules={[{ required: true, message: 'Required' }]}>
                    <Input prefix={<Key size={14} className="text-slate-400" />} placeholder="Your Meesho Supplier ID" />
                  </Form.Item>
