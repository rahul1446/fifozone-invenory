...
const AmazonAppPage = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await getOrdersApi({ platform: 'amazon', limit: 50 });
      const data = res?.data?.orders || res?.orders || (Array.isArray(res?.data) ? res.data : []);
      setOrders(data);
    } catch { message.error('Failed to fetch Amazon orders'); }
  ...
  const fetchStatus = async () => {
    try {
      ...
      setConnected(s?.amazon?.connected || false);
      if (s?.amazon?.sellerId) form.setFieldsValue({ sellerId: s.amazon.sellerId });
  ...
  const handleSave = async () => {
    try {
      ...
      setSaving(true);
      await updateCredentialsApi('amazon', vals);
      message.success('✅ Amazon SP-API credentials saved successfully!');
      setConnected(true);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 pb-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {connected === null && <span className="text-xs text-slate-400">Checking status...</span>}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Orders', value: orders.length, icon: <ShoppingBag size={20} className="text-orange-600" />, border: 'border-orange-400', bg: 'bg-orange-50' },
          { label: 'Total Revenue', value: `₹${totalRevenue.toLocaleString('en-IN')}`, icon: <IndianRupee size={20} className="text-amber-600" />, border: 'border-amber-400', bg: 'bg-amber-50' },
          ...
          { label: 'Pending', value: pending, icon: <ShoppingBag size={20} className="text-blue-600" />, border: 'border-blue-400', bg: 'bg-blue-50' },
        ].map(c => (
        ...
        <Tabs defaultActiveKey="orders" className="px-6 pt-2">
          <TabPane tab="📦 Orders" key="orders">
            {loading ? <div className="py-16 flex justify-center"><Spin size="large" /></div> :
              <Table columns={columns} dataSource={orders} rowKey="_id" pagination={{ pageSize: 15, showTotal: t => `${t} orders` }} locale={{ emptyText: '📭 No Amazon orders yet. Connect your API to start syncing.' }} />
            }
          </TabPane>
          ...
          <TabPane tab="🔑 API Settings" key="api">
            <div className="max-w-2xl py-4">
              <Alert
                message="Amazon Seller Partner (SP-API) Credentials"
                description="These credentials are required to automatically sync orders, inventory, and shipments from your Amazon Seller account."
                type="info"
                showIcon
                className="mb-6"
              />
              <Form form={form} layout="vertical" requiredMark={false}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
                  <Form.Item label={<span className="font-semibold text-slate-700">Seller ID</span>} name="sellerId" rules={[{ required: true, message: 'Required' }]}>
                    <Input prefix={<Key size={14} className="text-slate-400" />} placeholder="A2EUQ1WTGCTBG2" />
                  </Form.Item>
                  <Form.Item label={<span className="font-semibold text-slate-700">Marketplace ID</span>} name="marketplaceId">
                    <Input placeholder="A21TJRUUN4KGV (India default)" />
                  </Form.Item>
                  <Form.Item label={<span className="font-semibold text-slate-700">LWA Client ID</span>} name="clientId" rules={[{ required: true, message: 'Required' }]}>
                    <Input prefix={<Key size={14} className="text-slate-400" />} placeholder="amzn1.application-oa2-client..." />
                  </Form.Item>
                  ...
                  <Form.Item label={<span className="font-semibold text-slate-700">Refresh Token</span>} name="refreshToken" rules={[{ required: true, message: 'Required' }]} className="sm:col-span-2">
                    <Input.Password placeholder="Atzr|..." />
                  </Form.Item>
                  ...
                  <Form.Item label={<span className="font-semibold text-slate-700">AWS Secret Key</span>} name="awsSecretKey" rules={[{ required: true, message: 'Required' }]}>
