...
const AmazonAppPage = () => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const res = await getOrdersApi({ platform: 'amazon', limit: 20 });
    ...
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center text-2xl font-bold text-orange-600">A</div>
          <div><h1 className="text-2xl font-bold text-slate-800">Amazon Integration</h1><p className="text-slate-500 text-sm mt-1">Orders and performance from Amazon marketplace</p></div>
        </div>
        <Button icon={<RefreshCw size={16} />} onClick={fetchOrders}>Refresh</Button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total Orders', value: orders.length, icon: <ShoppingBag size={20} className="text-orange-600" />, border: 'border-orange-500', bg: 'bg-orange-50' },
          { label: 'Total Revenue', value: `\u20b9${totalRevenue.toLocaleString('en-IN')}`, icon: <IndianRupee size={20} className="text-amber-600" />, border: 'border-amber-500', bg: 'bg-amber-50' },
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 font-semibold text-slate-700">Recent Amazon Orders</div>
        <Table columns={columns} dataSource={orders} rowKey="_id" pagination={{ pageSize: 10 }} locale={{ emptyText: 'No Amazon orders found' }} />
      </div>
    </div>
  );
};
