...
import { Truck, Eye, Package, Inbox } from 'lucide-react';
...
const ShippedOrdersPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      render: (s) => (
        <span className={`px-3 py-1 text-[11px] font-bold rounded-full capitalize bg-indigo-50 text-indigo-600`}>
          {s?.replace('_', ' ') || 'Shipped'}
        </span>
      )
    },
    ...
    <div className="space-y-6 animate-fade-in pb-10 max-w-7xl mx-auto p-1">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-[22px] font-extrabold text-slate-900 tracking-tight">Shipped Orders</h1>
        </div>
        <p className="text-[13px] text-slate-400 font-medium ml-11">Orders currently in transit</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-[14px] p-5 border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">In Transit</p>
        ...
        <div className="bg-white rounded-[14px] p-5 border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Shipped Value</p>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-800 mb-1">No Shipped Orders</h3>
          <p className="text-sm text-slate-400 font-medium">Orders in transit will appear here once synced</p>
        </div>
      ) : (
