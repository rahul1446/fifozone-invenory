...
const CancelledOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      message.error('Failed to fetch cancelled orders');
      setOrders([]);
    } finally {
      setLoading(false);
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      render: (s) => (
        <span className={`px-3 py-1 text-[11px] font-bold rounded-full capitalize bg-rose-50 text-rose-600`}>
          {s?.replace('_', ' ') || 'Cancelled'}
        </span>
      )
    },
    {
    ...
    <div className="space-y-6 animate-fade-in pb-10 max-w-7xl mx-auto p-1">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-[22px] font-extrabold text-slate-900 tracking-tight">Cancelled Orders</h1>
        </div>
        <p className="text-[13px] text-slate-400 font-medium ml-11">Cancelled order history</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-800 mb-1">No Cancelled Orders</h3>
          <p className="text-sm text-slate-400 font-medium">Cancelled orders will appear here</p>
        </div>
      ) : (
